...
remove_short_audios() {
    ...
    _process_duration_removal "short" "$folder" "$threshold" "no"
}
...
remove_long_audios() {
    ...
    _process_duration_removal "long" "$folder" "$threshold" "no"
}
...
clean_filenames() {
    ...
    # Iterate over files, ignoring hidden ones
    for file in "$directory"/*; do
        if [ -f "$file" ]; then
            ...
            # Replace non-alphanumeric (except ._-) with nothing, and spaces with _
            # Using POSIX character class for better compatibility
            local newname
